...
class TrtConvertYoloBoxHeadTest(TrtLayerAutoScanTest):
    ...
    def sample_program_configs(self):
        ...
        class_num = 80
        batch_list = [1, 4]
        i_list = range(len(anchors))
        grid = [batch_list, i_list]
        for batch, i in itertools.product(*grid):
            attrs_dict = {'anchors': anchors[i], 'class_num': class_num}
            ops_config = [
                {
                    'op_type': 'yolo_box_head',
                    'op_inputs': {'X': ['yolo_box_head_input']},
                    'op_outputs': {'Out': ['yolo_box_head_output']},
                    'op_attrs': attrs_dict,
                }
            ]
                ...
                inputs={
                    'yolo_box_head_input': TensorConfig(
                        data_gen=lambda: generate_input(
                            attrs_dict, batch, input_shape[i]
                        )
                    )
                },

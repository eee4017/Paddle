...
void CheckFiniteAndUnscaleKernel(const Context& dev_ctx,
  ...
  const auto& cpu_place = phi::CPUPlace();
  const auto h_starts_size = (xs_size + 1) * sizeof(int64_t);

  // calculate each tensor's start index and copy to device
  auto h_starts_tensor = phi::memory_utils::Alloc(cpu_place, h_starts_size);
  int64_t* h_starts =
      phi::backends::gpu::RestoreHostMemIfCapturingCUDAGraph<int64_t>(
          reinterpret_cast<int64_t*>(h_starts_tensor->ptr()), h_starts_size);
  ...
  using target_type = T*;
  const auto h_xs_size = 2 * xs_size * sizeof(target_type);
  auto h_mem_allocation = phi::memory_utils::Alloc(cpu_place, h_xs_size);
  auto h_mem =
      phi::backends::gpu::RestoreHostMemIfCapturingCUDAGraph<target_type>(
          reinterpret_cast<target_type*>(h_mem_allocation->ptr()), h_xs_size);
  const T** h_xs = (const T**)(h_mem);
  T** h_outs = reinterpret_cast<T**>(h_mem) + xs_size;
  ...
  memory_utils::Copy(
      dev_ctx.GetPlace(), d_xs, cpu_place, h_xs, h_xs_size, dev_ctx.stream());
